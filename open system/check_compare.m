...
%         if mod(i-1,M) == 0
            ...
            H1 = H00 + K*sin(w*t_it).*diag([-1;1]) + envir;
            ...
            H2 = H00 + K*sin(w*(t_it+dt)).*diag([-1;1]) + envir;
            [V2,D2] = eig(H2);
            e2 = diag(D2);
            trans = V2'*phi;
...
t_it = 0;
H = Tij + diag(mu) + K*sin(w*t_it).*diag([-1;1]);
Et(1) = phi'*H*phi;
pos_mean = zeros(nt,1);
var_x2 = zeros(nt,1);
pos_mean(1) = wmean(x,abs(phi).^2,1);
