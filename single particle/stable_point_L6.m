...
ln = 1 / dn + 1;
ln_total = ln * (ln + 1) * (ln + 2) * (ln + 3) * (ln + 4) / 120;
result = cell(ln_total, sign_num);
result_t = cell(ln_total, sign_num);
Et_store = zeros(ln_total, sign_num);
...
it = it - 1;
summary = zeros(1,L);
summary_store = zeros(it,sign_num);

for i = 1:it

    for j = 1:sign_num
        [~, max_index] = max(result{i, j});
        summary_store(i,j) = max_index;
